...
# ========================
# Рекурсивное исправление двойной кодировки
# ========================
def safe_decode(obj):
    ...
    try:
        ...
        if act=='get_teachers':
            ...
            with db.cursor() as c:
                ...
                teachers=c.fetchall()
            db.close()
            return jsonify(safe_decode(teachers))
        ...
        if act=='get_groups':
            db=get_db()
            with db.cursor() as c:
                c.execute("SELECT g.id, g.group_name, u.login as curator_login, g.curator_id FROM groups g LEFT JOIN users u ON g.curator_id = u.id")
                groups=c.fetchall()
            db.close()
            return jsonify(safe_decode(groups))

        if act=='add_group' and request.method=='POST':
            if cur['role']!='admin': return jsonify({'error':'forbidden'}),403
            d=request.get_json()
            db=get_db()
            with db.cursor() as c:
                c.execute("INSERT INTO groups (group_name, curator_id) VALUES (%s,%s)",(d['group_name'], d['curator_id'] or None))
                db.commit()
            db.close()
            return jsonify({'ok':True})

        if act=='update_group' and request.method=='POST':
            if cur['role']!='admin': return jsonify({'error':'forbidden'}),403
            d=request.get_json()
            db=get_db()
            with db.cursor() as c:
                c.execute("UPDATE groups SET group_name=%s, curator_id=%s WHERE id=%s",(d['group_name'], d['curator_id'] or None, d['id']))
                db.commit()
            db.close()
            return jsonify({'ok':True})

        if act=='delete_group' and request.method=='POST':
            if cur['role']!='admin': return jsonify({'error':'forbidden'}),403
            d=request.get_json()
            db=get_db()
            with db.cursor() as c:
                c.execute("DELETE FROM groups WHERE id=%s",(d['id'],))
                db.commit()
            db.close()
            return jsonify({'ok':True})

        # ---- Clear all plans / events ----
        if act=='clear_all' and request.method=='POST':
            if cur['role']!='admin': return jsonify({'error':'forbidden'}),403
            db=get_db()
            with db.cursor() as c:
                c.execute("DELETE FROM events")
                c.execute("DELETE FROM plan_rows")
                c.execute("DELETE FROM plans")
                c.execute("DELETE FROM full_reports")
                db.commit()
            db.close()
            log_action(cur['id'], "Cleared all plans and events")
            return jsonify({'ok':True})
        ...
        if act=='get_my_last_plan':
            ...
            with db.cursor() as c:
                ...
                plan=c.fetchone()
            db.close()
            return jsonify({'plan': safe_decode(plan)})
        ...
        if act=='load_plan':
            ...
            with db.cursor() as c:
                ...
                rows=c.fetchall()
            db.close()
            return jsonify(safe_decode(rows))

        if act=='add_plan_row':
            if cur['role'] not in ('curator','admin'): return jsonify({'error':'forbidden'}),403
            d=request.get_json()
            dl=d['deadline']
            try: dl=datetime.strptime(dl,'%Y-%m-%d').strftime('%Y-%m-%d')
            except: pass
            db=get_db()
            with db.cursor() as c:
        ...
        if act=='update_plan_row':
            ...
            db=get_db()
            with db.cursor() as c:
                c.execute("UPDATE plan_rows SET module=%s, deadline=%s, responsible=%s, participants=%s, direction=%s WHERE id=%s",
                          (d['module'],dl,d['responsible'],d['participants'],d['direction'],d['id']))
                c.execute("UPDATE events SET title=%s, date=%s, direction=%s, participants=%s WHERE plan_row_id=%s",
                          (d['module'],dl,d['direction'],d['participants'],d['id']))
                db.commit()
            db.close()
            return jsonify({'ok':True})
        ...
        if act=='export_plan_docx':
            ...
            plan=safe_decode(plan); rows=safe_decode(rows)
            doc=Document()
        ...
        if act == 'events':
            ...
            # Гарантированное преобразование даты
            for ev in rows:
                raw_date = ev.get('date')
                # raw_date может быть datetime.date, datetime.datetime или строкой
                if hasattr(raw_date, 'strftime'):      # date или datetime
                    ev['date'] = raw_date.strftime('%Y-%m-%d')
                elif isinstance(raw_date, str):
                    try:
                        ev['date'] = datetime.strptime(raw_date, '%a, %d %b %Y %H:%M:%S %Z').strftime('%Y-%m-%d')
                    except:
                        pass
        ...
        if act=='get_social_passport':
            ...
            with db.cursor() as c:
                ...
                r=c.fetchone()
            db.close()
            return jsonify({'text': safe_decode(r)['content'] if r else ''})
        ...
        if act=='generate_report':
            ...
            with db.cursor() as c:
                ...
                good = safe_decode(c.fetchall())
                c.execute("SELECT * FROM events WHERE user_id=%s AND success='bad'",(cur['id'],))
                bad = safe_decode(c.fetchall())
                text = "САМОАНАЛИЗ КУРАТОРА\n\n"
                text += f"Всего запланировано мероприятий: {total}\n"
                text += f"Проведено: {done} ({pct}%)\n\n"
                if good: text += "Наиболее удачные:\n"+"\n".join(f"- {e['title']}" for e in good[:3])+"\n"
                if bad: text += "Неудачные:\n"+"\n".join(f"- {e['title']}" for e in bad[:3])+"\n"
        ...
        if act == 'ai_report':
            ...
            summary = f"Всего мероприятий: {total}, направления: {', '.join(dirs)}. Предоставь развёрнутый анализ работы куратора."

            access_token = get_gigachat_token()
            if not access_token:
                return jsonify({'report': '⚠️ Ошибка авторизации GigaChat'})
            ...
            try:
                r = requests.post(GIGACHAT_API_URL,
                    ...
                    json={
                        ...
                        'temperature':0.6,       # чуть меньше креативности, больше связности
                        'max_tokens': 1000       # было 150, теперь осмысленный объём
                    },
                ...
                if r.status_code == 200:
                    ...
                    return jsonify({'report': f'⚠️ Ошибка API GigaChat: {r.status_code}'})
            except Exception as e:
        ...
        if act=='get_dnevnik_grades':
            return jsonify({'students':[
                ...
                {'name':'Петрова Мария','subjects':[{'subject':'Математика','average':4.5,'marks':[5,4,5,4]}]}
            ], 'demo':True})
        ...
        if act=='get_my_full_report':
            ...
            with db.cursor() as c:
                ...
                rep=c.fetchone()
            db.close()
            return jsonify(safe_decode(rep) if rep else {})

        if act=='get_user_full_report':
            if cur['role'] not in ('organizer','admin'): return jsonify({'error':'forbidden'}),403
            uid=request.args.get('user_id'); period=request.args.get('period','2025-2026')
            db=get_db()
            with db.cursor() as c:
        ...
        if act=='export_report_docx':
            period=request.args.get('period','2025-2026'); text=request.args.get('text','')
            ...
            if pf:
                ...
                for i,h in enumerate(['Модуль','План','Факт','%']): table.rows[0].cells[i].text=h
        ...
        if act=='export_pdf':
            ...
            try:
                ...
                buf=BytesIO(); doc=SimpleDocTemplate(buf)
        ...
        if act=='summary_data':
            ...
            with db.cursor() as c:
                ...
                data=c.fetchall()
            db.close()
            return jsonify(safe_decode(data))
        ...
        if act=='get_user_plan_details':
            if cur['role'] not in ('organizer','admin'): return jsonify({'error':'forbidden'}),403
            uid=request.args.get('user_id'); year=request.args.get('year')
            db=get_db()
            with db.cursor() as c:
                c.execute("SELECT * FROM plans WHERE user_id=%s AND year=%s ORDER BY id DESC LIMIT 1",(uid,year))
                plan=c.fetchone(); rows=[]
                if plan:
                    c.execute("SELECT * FROM plan_rows WHERE plan_id=%s",(plan['id'],))
                    rows=c.fetchall()
            db.close()
            return jsonify({'plan':safe_decode(plan),'rows':safe_decode(rows)})
        ...
        if act=='get_templates':
            ...
            db.close()
            return jsonify(safe_decode(templates))
        ...
        if act=='delete_template':
            ...
            with db.cursor() as c: c.execute("DELETE FROM templates WHERE id=%s",(d['id'],)); db.commit()
            db.close()
            return jsonify({'ok':True})
        ...
        if act=='get_logs':
            ...
            with db.cursor() as c:
                ...
                logs=c.fetchall()
            db.close()
            return jsonify(safe_decode(logs))
